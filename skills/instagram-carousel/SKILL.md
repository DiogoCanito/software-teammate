...
### TeamMate Brand Defaults (pre-filled — do not ask the user for these)

| Field | Value |
|-------|-------|
| **Brand name** | TeamMate |
| **Logo** | `c:\Users\Diogo\Desktop\Claude Code\Software TeamMate\brand_assets\logo-teammate.svg` (embed as inline SVG or base64) |
| **Primary brand color** | `#05668d` (Blue) |
| **Secondary accent** | `#ff956b` (Coral) |
| **Heading font** | Funnel Display |
| **Body font** | Inter |
| **Tone** | Professional, modern, clean |
| **Idioma dos slides** | Português (pt-PT) |

Before generating, only ask the user for the following (if not already provided):

1. **Instagram handle** — shown in the IG frame header
2. **Images** — profile photo, screenshots, product images, etc.
3. **Carousel format** — standard (7 slides) or alternate sequence (see sequences section)
4. **Carousel topic / theme** — what the carousel is about

If the user says "make me a carousel about X" without specifying handle or images, ask only for those before generating.
...
### TeamMate — Pre-derived Palette (use these exact values, do not recalculate)

```
BRAND_PRIMARY   = #05668d   // Blue — progress bar, icons, tags
BRAND_LIGHT     = #0a8ab8   // Blue light — tags on dark slides, pills
BRAND_DARK      = #033d54   // Blue dark — CTA text, gradient anchor
BRAND_CORAL     = #ff956b   // Coral — secondary accent, highlights
LIGHT_BG        = #f4f7f9   // Cool off-white — light slide background
LIGHT_BORDER    = #dde3e8   // Dividers on light slides
DARK_BG         = #0a0a0a   // Near-black — dark slide background
```

Brand gradient: `linear-gradient(165deg, #033d54 0%, #05668d 50%, #0a8ab8 100%)`

**Rules for deriving colors (for non-TeamMate brands):**
...
### TeamMate — Pre-defined Typography (use these, do not substitute)

| Role | Font | Weight |
|------|------|--------|
| Heading (`.serif`) | **Funnel Display** | 700 |
| Body (`.sans`) | **Inter** | 400 / 600 |

Google Fonts import for TeamMate:
```
https://fonts.googleapis.com/css2?family=Funnel+Display:wght@400;600;700;800&family=Inter:wght@300;400;500;600;700&display=swap
```

**For non-TeamMate brands**, pick from this table based on user preference:
...
After generating the carousel HTML, **always** generate a caption and hashtags. Return them wrapped in the exact delimiters below — they are parsed programmatically.

### Instagram Caption
- Language: **Português (pt-PT)**
- Length: 150–200 words
- Structure: hook line (1 sentence that grabs attention) → body (expands on carousel theme, 3–4 sentences) → CTA paragraph (matches the CTA slide)
- Tone: professional but conversational, matching TeamMate brand voice
- Do NOT use emojis unless explicitly requested
- Include the CTA the user specified

### Hashtags
- 15–20 hashtags, mix of Portuguese and English (Portuguese-market focus)
- Include niche hashtags (lower competition), mid-tier, and broad hashtags
- Always include: `#teammatept #gestaoequipas #marketingdigital #ferramentasIA`
- Tailor remaining hashtags to the carousel topic

### Output Format (mandatory — parsed by backend)

Wrap the entire carousel HTML, caption, and hashtags in these exact delimiters:
...
**Important:** Do not include any text outside these delimiters in your response. The backend extracts content purely by regex on these tags.
